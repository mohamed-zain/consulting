...
@extends($ex)
@section('content')
<?php $pro = \App\Models\ActivateFile::where('Bennar',$i)->first(); ?>
...
- <span style="color: red">{{ $pro->FileCode }}</span>
                    </h3>

                    <div class="box-tools pull-right">
                        <a href="#" class="uppercase btn btn-primary" data-toggle="modal" data-target="#Modal">اضافة مهمة</a>
                    </div>
                    <!-- /.box-tools -->
                </div>
                <!-- /.box-header -->
...
<div class="modal fade" id="Modal" role="dialog">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">اضافة مهمة جديده </h4>
            </div>
            <div class="modal-body">
                {{ Form::open(array('route'=>'Tasks.store','id'=>'protepssend')) }}
                <div class="form-group">
                    <label>المشروع</label>
                    <input type="hidden" name="Bennar_Code" value="{{$pro->Bennar}}">
                    <input type="text" name="packageName2" class="form-control" placeholder="{{$pro->FileCode}}" value="{{$pro->FileCode}}" disabled>

                </div>
                <div class="form-group">
                    <?php $coddes = \App\Models\ProjectServices::where('Bennar_Code',$pro->Bennar )
                        //->leftJoin('tasks','project_services.Bennar_Code','=','tasks.Bennar_Code')
                        //->where('project_services.Bennar_Code','no')
                        ->where('status','no')
                        ->get()

                    ?>
                    <label>المهمة</label>
                    <select class="form-control select2" style="width: 100%" name="Mission">
                        @foreach($coddes as $item)
                            <option value="{{ $item->ServiceCode }}">{{$item->ServiceCode}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label>المهندس</label>
                    <select class="form-control select2" style="width: 90%" name="EmpID">
                        @foreach(\App\Models\User::where('Level','3' )->where('roll','AdmissionEmp')->get() as $item)
                            <option value="{{$item->id}}">{{$item->name}}</option>
                        @endforeach
                    </select>
                </div>
                <div id="shadu" style="">
                    <label>متي يتم فتح بطاقة العمل للمهندس:</label>
                    <div class="input-group date">
                ...
                <br>
                <label>متي يتم تسليم المهمة:</label>
                <div class="input-group date">
